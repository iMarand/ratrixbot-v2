...
input bool   InpUseADX        = false;// RSI+PA: REGIME FILTER -- trade only in ranges (ADX below max)
input ENUM_TIMEFRAMES InpADXTimeframe = PERIOD_D1; // timeframe for the regime ADX (higher = cleaner)
input int    InpADXPeriod     = 14;   // ADX period (regime filter)
input double InpADXMax        = 38.0; // skip RSI+PA when higher-TF ADX above this (trending = bad for mean reversion)
input bool   InpUseER         = false;// REGIME FILTER (data-driven): trade only when market moves cleanly
input ENUM_TIMEFRAMES InpERTimeframe = PERIOD_D1; // timeframe for the efficiency ratio
input int    InpERPeriod      = 20;   // lookback bars for efficiency ratio
input double InpERMin         = 0.12; // skip RSI+PA when efficiency ratio below this (too choppy)
...
int OnInit()
{
   ...
   adxHandle = iADX(_Symbol,InpADXTimeframe,InpADXPeriod); // regime gate on a higher TF
...
// Efficiency ratio over the last InpERPeriod bars of InpERTimeframe.
// 0 = pure chop, 1 = perfectly directional. Measures recent "cleanness".
double EfficiencyRatio()
{
   int p=InpERPeriod;
   double c[];
   if(CopyClose(_Symbol,InpERTimeframe,1,p+1,c) < p+1) return(1.0); // c[0]=oldest..c[p]=newest
   double net=MathAbs(c[p]-c[0]);
   double path=0; for(int i=1;i<=p;i++) path+=MathAbs(c[i]-c[i-1]);
...
int SignalRsiPa()
{
   ...
   // 3) Efficiency-ratio regime (data-driven): skip choppy markets, the strategy
   //    needs clean follow-through after entry.
   if(InpUseER && EfficiencyRatio() < InpERMin) return(0);
   return(dir);
}
